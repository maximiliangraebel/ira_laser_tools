...
public:
    ...
    bool getPointCloudAsEigenWithIntensity(const pcl::PCLPointCloud2 &in, Eigen::MatrixXf &out);

    const sensor_msgs::LaserScan add_laser_intensity(const sensor_msgs::LaserScan::ConstPtr &scan,
                                                     std::string topic);
...
void LaserscanMerger::laserscan_topic_parser() {
    ...
    std::string all_topics = laserscan_topics.append(" ").append(ultrasonic_topics);
    istringstream iss(all_topics);
    ...
    for (int i = 0; i < tokens.size(); ++i) {
        for (int j = 0; j < topics.size(); ++j) {
            if (tokens[i] == topics[j].name && topics[j].datatype == "sensor_msgs/LaserScan") {
    ...
    if ((tmp_input_topics.size() != input_topics.size()) ||
        ...
        if (!input_topics.empty()) {
            ...
            for (int i = 0; i < input_topics.size(); ++i) {
                scan_subscribers[i] = node_.subscribe<sensor_msgs::LaserScan>(input_topics[i], 1,
                                                                              boost::bind(
...
LaserscanMerger::LaserscanMerger() {
    ...
    nh.getParam("ultrasonic_topics", ultrasonic_topics);
    if (!nh.getParam("ultrasonic_intensity", ultrasonic_intensity))
        ultrasonic_intensity = 1;

    this->laserscan_topic_parser();

    point_cloud_publisher_ = node_.advertise<sensor_msgs::PointCloud2>(cloud_destination_topic, 1, false);
    laser_scan_publisher_ = node_.advertise<sensor_msgs::LaserScan>(scan_destination_topic, 1, false);
}
...
void LaserscanMerger::scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan, std::string topic) {
    ...
    const sensor_msgs::LaserScan scan_with_intensity = add_laser_intensity(scan, topic);


    // Verify that TF knows how to transform from the received scan to the destination scan frame
    tfListener_.waitForTransform(scan->header.frame_id.c_str(), destination_frame, scan->header.stamp,
                                 ros::Duration(1));
    projector_.transformLaserScanToPointCloud(scan->header.frame_id, scan_with_intensity, tmpCloud1, tfListener_);
    try {
    ...
    if (totalClouds == clouds_modified.size()) {
        ...
        getPointCloudAsEigenWithIntensity(merged_cloud, points);

        pointcloud_to_laserscan(points, &merged_cloud);
    }
}

const sensor_msgs::LaserScan LaserscanMerger::add_laser_intensity(const sensor_msgs::LaserScan::ConstPtr &scan,
                                                                  std::string topic) {
    /**adds laser intensity to fake laser scan from ultrasonic sensor*/
    sensor_msgs::LaserScan scan_with_intensity = *scan;
    if (ultrasonic_topics.find(topic) != std::string::npos) {
        scan_with_intensity.intensities.assign(scan->ranges.size(), ultrasonic_intensity); // 1 = sonar
    } else {
        scan_with_intensity.intensities.assign(scan->ranges.size(), 0); // 0 = laser
    }
    return scan_with_intensity;
}
...
void LaserscanMerger::pointcloud_to_laserscan(Eigen::MatrixXf points, pcl::PCLPointCloud2 *merged_cloud) {
    ...
    output->header.frame_id = destination_frame;
    ...
    for (int i = 0; i < points.cols(); i++) {
        ...
        if (intensity == ultrasonic_intensity) {
            output->intensities[index] = intensity;
        } else {
            output->intensities[index] = 0;
        }
    }
...
bool LaserscanMerger::getPointCloudAsEigenWithIntensity(const pcl::PCLPointCloud2 &in, Eigen::MatrixXf &out) {
    ...
    Eigen::Array4i xyz_offset(in.fields[x_idx].offset, in.fields[y_idx].offset, in.fields[z_idx].offset,
                              in.fields[intensity_idx].offset);
    // Eigen::Array4i xyz_offset (in.fields[x_idx].offset, in.fields[y_idx].offset, in.fields[z_idx].offset, 0); // oirg
    ...
    for (size_t i = 0; i < npts; ++i) {
        ...
        memcpy(&out(3, i), &in.data[xyz_offset[3]], sizeof(float));
